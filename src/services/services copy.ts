...
const createChapaPayment = async ({
  ...
  console.log({ CHAPA_TEST_SECRET_KEY });

  const CHAPA_URL = "https://api.chapa.co/v1/transaction/initialize";

  const CALLBACK_URL =
    process.env.NODE_ENV === "production"
      ? `https://api.jpstvethiopia.com/api/verify-payment/${tx_ref}`
      : `http://localhost:4000/api/verify-payment/${tx_ref}`;
...
export const createChapaPaymentForBookOrder = async ({
  first_name,
  last_name,
  phone,
  email,
  ...
  const CALLBACK_URL =
    process.env.NODE_ENV === "production"
      ? `https://api.jpstvethiopia.com/api/verify-book-purchase-payment/${tx_ref}`
      : `http://localhost:4000/api/verify-book-purchase-payment/${tx_ref}`;
